...
void COMMAND_DUMP(void)
{
	...
	//Write flash content to file
			...
			uint32_t flashEndAddr = FLASH_BASE + ((*((uint16_t*)FLASH_SIZE_DATA_REGISTER)) << 10);
			uint8_t error=0;
			uint8_t eof=0;
...
void COMMAND_FLASH(void)
{
	...
	//Check file size<available flash
		uint32_t bytesTotal=DATA_SIZE();
		const size_t availableFlashSize = ((*((uint16_t*)FLASH_SIZE_DATA_REGISTER)) << 10) - BOOTLOADER_FLASH_SIZE;
	...
	//Check flash is not write protected
			FLASH_OBProgramInitTypeDef OptionsBytesStruct;
			HAL_FLASHEx_OBGetConfig(&OptionsBytesStruct);
			if (((~OptionsBytesStruct.WRPPage) & FLASH_PAGE_TO_CHECK_WRP) == 0x00) //WRPPage bits are 1 when not protected
			{
	//Erase pages required by update
				FLASH_EraseInitTypeDef EraseInitStruct;
				EraseInitStruct.TypeErase   = FLASH_TYPEERASE_PAGES;
				EraseInitStruct.PageAddress = APPLICATION_ADDRESS;
				EraseInitStruct.NbPages     = (bytesTotal/FLASH_PAGE_SIZE)+1; //erase up to the file size instead of all pages
	...
	//Program the update
					...
					while (!error && !eof)
					{
						...
						{
							if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, flashAddr, *((uint16_t*)(RAM_Buf+bufOffset))) == HAL_OK)
							{
								flashAddr = flashAddr + 2;
								bufOffset = bufOffset + 2;
							}else{
...
//The following generates a hardfault
//        USBH_Stop(&hUsbHostFS);
//        USBH_DeInit(&hUsbHostFS);
//        USBH_LL_DeInit(&hUsbHostFS);
//...And all of these don't help either
//   	MX_FATFS_DeInit();
//   	MX_USB_HOST_DeInit();
//    	(void)USB_DisableGlobalInt(USB_OTG_FS);
//		__disable_irq(); //Do NOT disable IRQs because they are not reactivated by the App
//		__DSB();
//...However on my STM32F105 it seems that release bootloader binaries work without this (but not debug ones)
